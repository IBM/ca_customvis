...
export default class extends RenderBase
{
    ...
    protected update( _info: UpdateInfo ): void
    {
        ...
        if ( data.cols[ SERIES ].mapped )
        {
            ...
            const categories = data.cols[ ITEM ].tuples.map( e => e.caption );
            ...
            categories.forEach( _category =>
                {
                    // for each category, create an array of series.length
            ...
            data.rows.forEach( _row =>
                {
                    chartData[ categories.indexOf( _row.tuple( ITEM ).caption ) + 1 ][ series.indexOf( _row.tuple( SERIES ).caption ) ] = _row.value( VALUE );
                } );
        }
        else
        {
            // header row
            chartData.push( [ data.cols[ ITEM ].caption, data.cols[ VALUE ].caption ] );
            // add data
            data.rows.forEach( _row =>
            {
                chartData.push( [ _row.tuple( ITEM ).caption, _row.value( VALUE ) ] );
            } );
        }
        ...
        // Set google chart options via properties and palette
        ...
        const options: any = {
            ...
            vAxis: {
                ...
                minValue: props.get( "vAxis.minValue" ),
                maxValue: props.get( "vAxis.maxValue" ),
                baselineColor: props.get( "vAxis.baseline.color" ).toString(),
                gridlines: { color: props.get( "vAxis.grid.color" ).toString() }
            },
            chartArea: {
            ...
            histogram: {
                hideBucketItems: !props.get( "histogram.showItems" ),
                bucketSize: props.get( "histogram.bucketSize" ),
                lastBucketPercentile: props.get( "histogram.ignorePercentile" )
            },
            isStacked: props.get( "series.type" ) === "stacked",
            legend: {
