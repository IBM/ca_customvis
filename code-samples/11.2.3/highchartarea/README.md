...
Because of licensing limitations, the packed customvis loads the highcharts library from a CDN instead of bundling the highcharts library. If the sample is built from this repository, the highcharts library will be boundled in customvis package. Before use make sure to aquire a licence from Highcharts.
