...
const POINTS = 0, TOP = 1, LEFT = 2, RIGHT = 3, COLOR = 4, SIZE = 5; // data slot indices
...
export default class TernaryChart extends RenderBase
{
    ...
    private readonly ternary: Ternary = new Ternary();
    ...
    protected create( _node: HTMLElement ): void
    {
        ...
        this.ternary.topValue = _d => _d.value( TOP ); // get top value from data point
        this.ternary.leftValue = _d => _d.value( LEFT ); // get left value from data point
        this.ternary.rightValue = _d => _d.value( RIGHT ); // get right value from data point
        this.ternary.sizeValue = _d => _d.value( SIZE ); // get size value from data point
    ...
    protected update( _info: UpdateInfo ): void
    {
        ...
        if ( _info.data && _info.data.cols[ SIZE ].mapped )
            this.ternary.pointSize = d3.scaleLinear()
            ...
            this.ternary.pointSize = props.get( "point_size" );
        this.ternary.width = _info.node.clientWidth;
        this.ternary.height = _info.node.clientHeight;
        this.ternary.labelPlacement = _info.props.get( "point_label_placement" );

        // Create a layout object that holds everything we need for rendering.
        const layout = this.ternary.createLayout();
                          ...
                          _layout: TernaryLayout ): void
{
    ...
    const anchor = labelPos.indexOf( "Left" ) != -1 ? "start" : "end";
...
function showHidePointLabels( _sel: d3.Selection<SVGTextElement, any, any, any>, _info: UpdateInfo )
{
