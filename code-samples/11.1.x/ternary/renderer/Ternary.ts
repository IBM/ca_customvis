...
export class Ternary
{
    ...
    public width: number = 0; // width of the container
    public height: number = 0; // height of the container
    public titleMargin: number = 50; // number of pixels around the chart reserved for titles
    ...
    public createLayout(): TernaryLayout
    {
        ...
        {
            let v0 = this.topValue( _d ), v1 = this.leftValue( _d ), v2 = this.rightValue( _d );
        ...
        const labelXFn = _d => circleXFn( _d ) + dX * circleRFn( _d ) * Math.SQRT2 / 4;
        const labelYFn = _d => circleYFn( _d ) + dY * circleRFn( _d ) * Math.SQRT2 / 4;
